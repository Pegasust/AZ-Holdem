...
private:
  ...
  static Optional<int> straight_highest_card_idx(const std::vector<Card>& cards) {
    for (int i = 1; i < 4; ++i)
    {
    ...
    int highest_card = low_ace? (cards.size()-2): (cards.size()-1);
    return make_optional(highest_card);
  }
